...
void main(){
    ...
    uv.y += iTime * .3;
    uv.y += sin(iTime + uv.y) * .3;
    
    uv.y += sin(iTime + uv.x) * .3;
    
    uv.x += sin(iTime) * .1;
    
    
    float wiggle = .15 + .15 * snoise(vec3(uv * vec2(1,3.), iTime * .3));
    
    
    uv += wiggle * snoise(vec3(uv, -iTime * .25));
     
    vec3 voronoi = voronoiDiagram(uv, iTime * .1);

    vec3 col = vec3(.07, .7, 1.);
    
    
    float thickness = .15;
    thickness += snoise(vec3(uv * .5, iTime * .1)) * .18;
    thickness += snoise(vec3(uv * 3., -iTime * .5)) * .10;
    thickness += snoise(vec3(uv * 50., -iTime * 2.)) * .02;
    
    
    float border = smoothstep(voronoi.x, voronoi.x+.35, thickness);
    col += border * vec3(.3,.2,0);
    
    float middle =  .3 * smin(.1,snoise(vec3(uv * .5, iTime * .25)) + 1., .95);
